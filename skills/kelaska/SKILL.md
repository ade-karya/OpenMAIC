...
After the core LLM key is configured, ask the user if they want to enable optional features (web search, image generation, video generation, TTS, ASR). Each requires its own provider key — see the "Optional Features" section in provider-keys.md.

If the user wants to support multiple users with isolated API keys, direct them to the "PIN Users" section in provider-keys.md.
