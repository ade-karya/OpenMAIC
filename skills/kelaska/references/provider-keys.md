...
KelasKA generation does not automatically reuse the OpenClaw agent's current model or API key.

KelasKA server APIs resolve their own model and provider keys from KelasKA server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit KelasKA server-side config files.
...
- KelasKA server fallback is currently `gpt-4o-mini` if `DEFAULT_MODEL` is unset.
...
Do not recommend bare model IDs such as `gemini-3-flash-preview` by themselves, because KelasKA will otherwise parse them as OpenAI models.
...
- "I recommend configuring KelasKA through `.env.local` first. Please edit that file locally and tell me when you're done."
